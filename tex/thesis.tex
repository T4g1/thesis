...
I tried to divided this work in many big parts that introduces themself one after the other. As this topic and all thetechniques involved are closely related, there will often be references to area not already covered during the sequential reading of this paper.\\

To begin with, we will present the very basics of one software testing by analyzing one of them mainly to introduce technical terms used all along this paper. Following this technical introduction, we will present the studied case that was the starting point of this work at the RailNOVA company. Then, we will start with the real work by explaining how we designed the testing process and finally, we will describe how our particular case was handled by explaining how we integrated every inuts and outputs of our software into our testing process. We'll also give a more summarized/generic way of doing the same work for any kind of inputs and outputs.
...
This whole process, starting from the client order to the shipping of the produced software is what we call the software development process. Of course, this is not the only way of doing it even for a simple process like this one their can be divergence of workflow from one team to another. But lets keep our simple view of it and introduces the tools that are of interest for every testing process.
...
Now that you now what software testing is all about, lets introduce the case that is of interest to us, namely, Nursery ! I'll first introduce the RailNOVA enterprise and the system we will work with all along this paper. Then I will explain why this work is of critical importance for them and generaly speaking, for every software development process.

\clearpage
\section{RailNOVA}

This work is done in partnership with RailNOVA. Railnova is a technology company that provides telematics and software solutions to the railway industry. Their headquarter is located in Brussels and they count approximatively 30 employees. Some of them comes from the ULB/VUB.
...
This first idea gave us an important lesson, we need to keep track of the state of the process or at least of what hapenned before the current time. This does correspond to what we call \gls{fsm}. So let's introduce them and some expansions of \gls{fsm} that seems pertinent in our case.\\
...
\section{Alternate Finite Automaton}
...
\section{Timed Finite State Machine}
...
\section{Mixing alternate and timed automata}

TODO DISCUSSION WITH MR GEERAERTS HERE

\section{Choices}

For certainty, there are a lot of available possibility when it comes to designing an abstract machine. Some of them are purely theoritical abstract machine so we wont even mention them as we need only a practical implementation, and some of them are simply not adapted to our case. Lets review them all and justify the choices we made.\\

First of all, let's explain what are the main differences between our practical case and theory. In theorical \gls{fsm}, the number of state is upper bound by the system's internal memory. If we want to test every possible state it would gives us $2^x$ states to analyze (where $x$ is the system's internal memory expressed in bit). In modern computers, this memory can be around two to height gigabits maybe and this is raising every passing years. So having to describe what should happen in $2^2*10^9$ different states is not really a possible choice.\\
...
\clearpage
\part{Abstraction of Inputs/Outputs}
